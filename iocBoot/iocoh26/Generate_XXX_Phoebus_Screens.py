...
def find_bob_files(base_dir, dest_dir):
    """Find all .bob files under base_dir, excluding the staging dir."""
    bob_files = []
    for root, _, files in os.walk(base_dir):
        ...
        for file in files:
            if file.endswith(".bob"):
                bob_files.append(os.path.join(root, file))
    return bob_files

def create_symlink(bob_file, dest_dir, env_key=None):
    """Create or update a symlink for a .opi file in dest_dir."""
    basename   = os.path.basename(bob_file)
    dest_file  = os.path.join(dest_dir, basename)
    new_target = os.path.realpath(bob_file)  # always resolve to real file
...
def process_path(env_key, path, dest_dir):
    ...
    bob_files = find_bob_files(path, dest_dir)
    for bob_file in bob_files:
        create_symlink(bob_file, dest_dir, env_key)
    return bob_files
...
#            print(f"Scanning [{key}] → {path}")
    ...
    print(f"Total .bob files linked/checked: {len(all_bob_files)}")
...
if __name__ == "__main__":
    ...
    dest_dir = os.path.abspath(os.path.join(envpaths_dir, "..", "..", "xxxApp","op", "bob","autoconvert" ))
